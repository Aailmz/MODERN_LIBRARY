...
<body class="g-sidenav-show bg-gray-800">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto  max-height-vh-100 h-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{ route('tablenotification') }}">
              <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <svg width="12px" height="12px" viewBox="0 0 45 40" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                  <title>shop </title>
                  <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
      ...
      <div id="tablesiswa">
            ...
            <div class="card-body px-0 pt-0 pb-2 bg-gray-200">
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0" id="tablesiswa">
                        <thead>
                            <tr>
                                <th class="text-uppercase text-white text-xxs font-weight-bolder">Name</th>
                                <th class="text-uppercase text-white text-xxs font-weight-bolder">Email</th>
                                <th class="text-uppercase text-white text-xxs font-weight-bolder">Book Title</th>
                                <th class="text-uppercase text-white text-xxs font-weight-bolder">Book Category</th>
                                <th class="text-uppercase text-white text-xxs font-weight-bolder">Borrow Duration</th>
                        ...
                        <tbody>
                            ...
                            <tr id="notification_{{ $notification->id }}">
                                ...
                                <td>{{ $notification->borrow_duration }}</td>
                                <td>
                                    <button type="button" class="btn bg-gradient-warning w-30 mt-0 mb-0" onclick="showRejectModal('{{ $notification->id }}')">Reject</button>
                                    <button type="button" class="btn bg-gradient-success w-30 mt-0 mb-0" onclick="acceptNotification('{{ $notification->id }}')">Accept</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal fade" id="rejectModal" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <h5 class="modal-title" id="rejectModalLabel">Reject Notification</h5>
  ...
  <script>
    ...
    function submitRejectForm() {
        ...
        // Call the rejectNotification function with the form data
        rejectNotification(notificationId, condition, note);
    }

    function rejectNotification(notificationId, condition, note) {
        Swal.fire({
            title: "Reject Loan?",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("reject-notification") }}', // Laravel helper to generate route URL
                    data: {
                        id: notificationId,
                        condition: condition,
                        note: note
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            title: "Success",
                            text: "Loan is rejected and notes are sended!",
                            icon: "success"
                        }).then((result) => {
                            if (result.isConfirmed) {
                                location.reload();
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "An error occurred while rejecting the notification.",
                            icon: "error"
                        });
                    }
                });
            } else if (result.isDenied) {
                // Handle the case where the user denied the action
            }
        });
    }
    ...
    function acceptNotification(notificationId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("accept-notification") }}', // Laravel helper to generate route URL
                    data: {
                        id: notificationId,
                    },
                    headers: {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "An error occurred while accepting the notification.",
                            icon: "error"
                        });
                    }
                });
